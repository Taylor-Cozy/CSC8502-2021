...
Mesh* Mesh::GenerateCircle() {
	...
	m->tangents[0] = Vector4(1.0f, 0.0f, 0.0f, 1.0f);
...
void UploadAttribute(GLuint* id, int numElements, int dataSize, int attribSize, int attribID, void* pointer, const string&debugName) {
	...
	glBufferData(GL_ARRAY_BUFFER, numElements * dataSize, pointer, GL_DYNAMIC_DRAW);
...
void	Mesh::BufferData()	{
	...
	if (weightIndices) {
		...
		glBufferData(GL_ARRAY_BUFFER, numVertices * sizeof(int) * 4, weightIndices, GL_DYNAMIC_DRAW);
