...
SceneNode::SceneNode(Mesh* mesh, Vector4 colour, bool shadowExempt) : shadowExempt(shadowExempt) {
	this->mesh = mesh;
	this->colour = colour;
	parent = NULL;
...
void SceneNode::SetShaderVariables() {
	...
	glUniform1i(glGetUniformLocation(shader->GetProgram(), "useTexture"), useTexture);
